<?php

namespace App\Http\Controllers\School;

use App\Http\Controllers\Controller;
use App\Models\Certificate;
use App\Models\CertificateTemplate;
use App\Models\CourseEnrollment;
use App\Models\School;
use App\Services\Certificates\CertificateGenerator;
use Illuminate\Http\RedirectResponse;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Storage;
use Inertia\Inertia;
use Inertia\Response;
use Symfony\Component\HttpFoundation\BinaryFileResponse;
use Symfony\Component\HttpFoundation\StreamedResponse;

class CertificateController extends Controller
{
    public function index(School $school): Response
    {
        $certificates = Certificate::query()
            ->whereHas('course', fn ($query) => $query->where('school_id', $school->id))
            ->with(['user:id,name', 'course:id,title'])
            ->latest('issued_at')
            ->limit(50)
            ->get()
            ->map(fn (Certificate $certificate): array => [
                'id' => $certificate->id,
                'number' => $certificate->certificate_number,
                'holder' => $certificate->user?->name ?? '—',
                'course' => $certificate->course?->title ?? '—',
                'issued' => optional($certificate->issued_at)->toDateString() ?? '—',
                'expires' => optional($certificate->expires_at)->toDateString() ?? '—',
                'status' => ucfirst($certificate->status ?? 'valid'),
            ]);

        $templates = CertificateTemplate::query()
            ->where('is_active', true)
            ->orderByDesc('is_default')
            ->get(['id', 'name', 'slug', 'is_default']);

        return Inertia::render('school/certificates/index', [
            'certificates' => $certificates,
            'templates' => $templates,
        ]);
    }

    public function store(Request $request, School $school, CertificateGenerator $generator): RedirectResponse
    {
        $validated = $request->validate([
            'course_enrollment_id' => ['required', 'exists:course_enrollments,id'],
            'certificate_template_id' => ['nullable', 'exists:certificate_templates,id'],
        ]);

        $enrollment = CourseEnrollment::query()
            ->whereHas('course', fn ($query) => $query->where('school_id', $school->id))
            ->findOrFail($validated['course_enrollment_id']);

        $template = isset($validated['certificate_template_id'])
            ? CertificateTemplate::query()->find($validated['certificate_template_id'])
            : null;

        $generator->issue($enrollment, $template);

        return back()->with('success', 'Certificate issued.');
    }

    public function download(School $school, Certificate $certificate): BinaryFileResponse|StreamedResponse
    {
        $certificate->loadMissing('course');

        abort_unless($certificate->course?->school_id === $school->id, 403);

        if (blank($certificate->pdf_path) || ! Storage::disk('local')->exists($certificate->pdf_path)) {
            app(CertificateGenerator::class)->renderFiles($certificate);
            $certificate->refresh();
        }

        return Storage::disk('local')->download(
            $certificate->pdf_path,
            $certificate->certificate_number.'.pdf',
        );
    }
}
